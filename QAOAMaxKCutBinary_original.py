...
from Utilities import Cn_U3_0theta0
...
class QAOAMaxKCutOnehot(QAOABase):
    ...
    def binstringToLabels_MaxKCut(self, k_cuts,num_V,binstring): # converts a string of size n (binstring) into a k-partition string
        k_bits = self.kBits_MaxKCut(k_cuts)
        label_list = []
        for j in range(num_V):
            for i in range(j*k_bits,(j+1)*k_bits):
                label_list += [int(binstring[i])]
